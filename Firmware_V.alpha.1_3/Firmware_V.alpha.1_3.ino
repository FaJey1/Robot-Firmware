...
class HeadRotate // реализация изменения угла поворота
{
  ...
  {
      if( (button == 70 || button == '4') && angleRotate!=angleTop )
    {
        angleRotate+=5;
    }
    else
    {
      ...
      if( (button == 72 || button == '5') && angleRotate!=angleLow )
    {
        angleRotate-=5; 
    }
    else
...
void loop() {
   ...
   {*/
       ...
       button[12] = (int)byteButton[12];
       for(int i = 0; i < 12; i++){
       Serial.write(button[i]);
       }
   if(button[6] == 70 || button[8] == 72 )
   {
      angleLimiterTop = 160;
      angleLimiterLow = 10;
      if(button[6] == 70 && button[8] == 0){
        buttonPer = button[6];
      }
      if(button[6] == 0 && button[8] == 72){
        buttonPer = button[8];
      }
      angleNowRightLeft1=HeadServo1.headRotate(buttonPer, angleLimiterTop, angleLimiterLow);
      servoHeadRotateRightLeft1.write(angleNowRightLeft1);
      delay(150);
   }
   if(button[5] == '1' || button[7] == '2' )
   {
      if(button[5] == 71 && button[7] == 0){
        buttonPer = button[6];
      }
      if(button[5] == 0 && button[7] == 84){
        buttonPer = button[8];
      }
      angleLimiterTop = 135;
      angleLimiterLow = 30;
      angleNowUpDown=HeadServo3.headRotate(buttonPer, angleLimiterTop, angleLimiterLow);
